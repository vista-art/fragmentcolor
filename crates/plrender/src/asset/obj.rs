...
pub fn load_obj(
    ...
    for object in obj.data.objects {
        for group in object.groups {
            ...
            let mut renderable = scene.new_renderable(&bundle);
            renderable.set_parent(node);
            ...
            if let Some(obj::ObjMaterial::Mtl(ref mat)) = group.material {
                if let Some(cf) = mat.kd {
                    ...
                    renderable.add_component(crate::Color(color));
                }
                if !normals.is_empty() {
                    if let Some(glossiness) = mat.ns {
                        renderable.add_component(renderer::renderpass::Shader::Phong {
                            glossiness: glossiness as u8,
                        })
                    } else {
                        renderable
                            .add_component(renderer::renderpass::Shader::Gouraud { flat: false })
                    };
                }
            }

            let renderable_entity = scene.add(&mut renderable);

            entities.insert(group.name, renderable_entity);
        }
    }
