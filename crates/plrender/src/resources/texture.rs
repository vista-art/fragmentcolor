...
impl Default for TextureId {
    fn default() -> Self {
        ...
        let (texture_id, _size) = Texture::from_file(default).expect("Default image not found!");
        texture_id
    }
}
...
impl Texture {
    ...
    pub fn from_file(path: impl AsRef<Path>) -> Result<(TextureId, Quad), Error> {
        ...
        let texture_id = Self::from_image(&image)?;
    ...
    fn from_image(image: &DynamicImage) -> Result<TextureId, Error> {
    ...
    pub fn create_depth_texture(size: wgpu::Extent3d) -> Result<TextureId, Error> {
