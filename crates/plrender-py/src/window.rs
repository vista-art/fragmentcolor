...
//
// PLRender can't send filenames directly to callbacks because
        ...
        if let Ok(windows) = app.windows().try_read() {
            if let Some(window) = windows.get(&window_id) {
                if let Some(filename) = window.get_hovered_file(handle) {
                    ...
                    app.error("plrender-py: Cannot read filename: ignoring dropped file!");
                    "None".to_string()
                }
            } else {
                app.error("plrender-py: Cannot read Window: ignoring hovered file!");
                "None".to_string()
            }
        } else {
            app.error("plrender-py: Cannot read Windows list: ignoring hovered file!");
            "None".to_string()
        }
    } else {
    ...
    if let Ok(app) = PLRender::app().try_read() {
        if let Ok(mut windows) = app.windows().try_write() {
            if let Some(mut window) = windows.get_mut(&window_id) {
                ...
                app.error("plrender-py: Cannot read Window: ignoring dropped file!");
                "None".to_string()
            }
        } else {
            app.error("plrender-py: Cannot read Windows list: ignoring dropped file!");
            "None".to_string()
        }
    } else {
