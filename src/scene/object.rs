...
//! [`Light`](crate::Light) all implement it; together they cover the three
//! shapes any 3D scene needs (geometry, viewpoint, illumination). The trait
//! mirrors the glTF / USD node model — every "thing that lives in a scene"
//! is a node attached to its parent.
//!
...
//! Camera and Light use this hook for live propagation; Model doesn't need
//! it (its work is fully done at attach time).
...
pub trait SceneObject: std::fmt::Debug + Send + Sync + 'static {
    ...
    /// Default is a no-op; Camera and Light override it to write their
    /// current state into the new shader so order of attachment doesn't
    /// matter. Model doesn't need this — its work is one-shot at attach.
