...
fn js_shader_example_uses_template_literal_and_no_rust_raw_markers() {
    ...
    assert!(src.contains("new Shader(`"), "JS should use template literal with backticks");
    assert!(!src.contains("r#\""), "JS must not contain Rust raw string opener");
    assert!(!src.contains("\"#"), "JS must not contain Rust raw string closer");

    // Ensure we didn't mutate WGSL to add spurious semicolons (spot-check @vertex;)
    assert!(!src.contains("@vertex;"), "WGSL must remain unmodified (no added semicolons)");
}
...
fn py_shader_example_uses_triple_quotes_and_no_leading_space_before_comment() {
    ...
    assert!(src.contains("Shader(\"\"\""), "Python should use triple-quoted string for WGSL");
    assert!(!src.contains("r#\""), "Python must not contain Rust raw string opener");
    assert!(!src.contains("\"#"), "Python must not contain Rust raw string closer");
