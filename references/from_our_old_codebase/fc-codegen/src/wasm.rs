...
pub(crate) fn wrap(struct_ident: Ident, object_properties: &[ObjectProperty]) -> TokenStream {
    let struct_name = struct_ident.to_string();
    let wrapper_name = format!("Js{}", struct_name);
    let wrapper_ident = syn::Ident::new(&wrapper_name, struct_ident.span());

    // generate WASM wrappers
    let methods = object_properties
        .iter()
        .map(|property| {
            let name = syn::parse_str::<syn::Ident>(&property.name).unwrap();
            let setter_name = format!("set_{}", name);
            let setter = syn::Ident::new(&setter_name, name.span());
            let type_name = syn::parse_str::<syn::Type>(&property.type_name).unwrap();
            ...
            if let Some(signature) = &property.function {
                ...
                let return_type = if signature.return_type.is_some() {
                    let return_type =
                        syn::parse_str::<syn::Type>(&signature.return_type.as_ref().unwrap())
                            .unwrap();
                ...
                quote! {
                    ...
                    fn #setter(&mut self, value: #type_name) {
