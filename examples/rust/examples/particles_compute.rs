...
fn main() {
    ...
    let cs = Shader::new(&cs_src).expect("compute shader");
    let fs = Shader::new(&fs_src).expect("render shader");
    ...
    app.on_resize(on_resize)
        ...
        .on_redraw_requested(move |_app| {
            // Update only sim uniform (optional tweak)
            // You can animate dt or g here if desired; keeping constant for stability
