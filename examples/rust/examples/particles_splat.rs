use fragmentcolor::{App, Frame, Pass, Renderer, SetupResult, Shader, TextureFormat, call, run};
use std::sync::Arc;
...
fn make_splat_wgsl(n: u32, size: u32) -> String {
    let tpl = r#"
const N: u32 = __N__u;
...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // Render each particle as a SIZExSIZE square
  for (var dx = 0u; dx < SIZE; dx++) {
    for (var dy = 0u; dy < SIZE; dy++) {
      let x = px + i32(dx);
      let y = py + i32(dy);
...
// Create a render target for each window on startup so render passes have a destination
...
fn main() {
    ...
    let num_particles: u32 = std::env::var("PARTICLES")
        .ok()
    ...
    // Shaders
    let cs_update = Shader::new(&make_update_wgsl(num_particles)).expect("update shader");
    let cs_clear = Shader::new(CLEAR_WGSL).expect("clear shader");
    let cs_splat =
        Shader::new(&make_splat_wgsl(num_particles, particle_size)).expect("splat shader");
    let fs_present = Shader::new(FS_PRESENT_WGSL).expect("present shader");

    let particles = num_particles as usize;
    let mut positions = vec![0f32; particles * 2];
    let mut velocities = vec![0f32; particles * 2];
    let mut colors = vec![0f32; particles * 4];
    ...
    cs_update
        .set("positions", bytemuck::cast_slice(&positions))
        .expect("set pos");
    cs_update
        .set("velocities", bytemuck::cast_slice(&velocities))
        .expect("set vel");
    ...
    cs_splat
        .set("colors", bytemuck::cast_slice(&colors))
        .expect("bind colors");
    ...
    let wx = num_particles.div_ceil(256).max(1);
    ...
    // Store into registry for callbacks
    app.add("shader.update", cs_update.clone());
    app.add("frame.main", {
        let mut f = Frame::new();
        f.add_pass(&pass_update);
        f.add_pass(&pass_clear);
        f.add_pass(&pass_splat);
        f.add_pass(&pass_present);
        f
    });
