---
title: Light
description: "`Light` is the unified type covering the three glTF `KHR_lights_punctual` kinds: **directional** (parallel rays — sun / sky / fill), **point** (radiates from a position with inverse-square falloff — lamps / candles / fireballs), and **spot** (point light constrained to a cone — flashlights / headlamps / theatre lighting). The kind is set once at construction via [`Light::directional`](/api/scene/light/directional), [`Light::point`](/api/scene/light/point), or [`Light::spot`](/api/scene/light/spot)."
category: scene
categoryLabel: Scene
sidebar:
  order: 4
...
- **Universal** — [`set_color`](/api/scene/light/set_color)
  and [`set_intensity`](/api/scene/light/set_intensity)
  apply to every kind and return `Self` for chaining.
- **Kind-specific** — [`set_position`](/api/scene/light/set_position),
