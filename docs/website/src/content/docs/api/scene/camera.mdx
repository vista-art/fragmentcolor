---
title: Camera
description: "A `Camera` packages the two things every 3D render needs into one object: a projection (how the view frustum maps to clip space) and a view (where the camera sits and what it looks at). Pass it to [`Material::add`](/api/scene/material#add) to wire its `camera.view_proj` and `camera.position` into the material's shader; the Camera holds Arc-shared state, so subsequent [`look_at`](/api/scene/camera/look_at) calls propagate to every Material that absorbed it."
category: scene
categoryLabel: Scene
sidebar:
  order: 2
...
[`Material::add`](/api/scene/material#add) to wire
