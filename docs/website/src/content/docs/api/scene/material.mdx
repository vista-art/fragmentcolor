...
each one to a 1Ã1 default from its lazy texture cache the first time a Pass
...
available at build time â they ship in the default `shaders-all` feature,
...
The "many handles to one shader" share semantics is the right default â
...
- `AlphaMode::Blend` â depth-test stays on but depth-write turns off, and the color
  ...
  Cross-Material interleaving works correctly â translucent draws across every shader
...
[Material â Cloning is an Arc-share](/api/scene/material#cloning-is-an-arc-share-not-a-deep-copy)
...
- A pre-built [`&Texture`](/api/texture/texture) â the eager path. The setter stores the texture's ID immediately and the GPU texture stays Arc-shared across every Material that points at it. Passing the same `&shared_texture` to N Materials produces one GPU upload + N shader-uniform references â the cheap way to wallpaper a brush stroke across a scene full of impasto blobs without paying for N texture allocations.
- Bytes / path / URL / `DynamicImage` â the lazy path. The setter queues the input on the Material's Shader; the renderer drains queued uploads on the first [`Renderer::render`](/api/core/renderer/render) (or earlier via the explicit [`Renderer::load`](/api/core/renderer/load)).
...
The setter itself is infallible â it queues the upload (lazy path) or
takes an Arc-clone (eager path) and returns. Failures from the lazy
path â file not found, decode error, unsupported format â surface when
...
Until then the Material renders against its 1Ã1 default and a
...
// 279 blob Materials all sample the same uploaded `albedo` â one GPU
...
fn main() -> Result<(), Box<dyn std::error::Error>> { pollster::block_on(run()) }
`}
lang="rust" meta="collapse={1-1, 18-21}"
/>
...
resolves each one to a sensible 1Ã1 default from its lazy texture cache the
...
to [`Pass::add`](/api/core/pass#add) â the
