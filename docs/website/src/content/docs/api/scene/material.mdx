...
`}
lang="rust" meta="collapse={1-1, 19-22}"
/>
...
Absorb a [Camera](/api/scene/camera) or
[Light](/api/scene/light) (or any custom
[`Component`](/api/scene)) into the Material's
...
// Updating the camera later is enough â the Material picks the new
...
fn main() -> Result<(), Box<dyn std::error::Error>> { pollster::block_on(run()) }
`}
lang="rust" meta="collapse={1-1, 16-19}"
/>
...
lazily â so calling `Material::pbr(&renderer).await?` without binding a
...
renderer = Renderer()
detailed = Material.pbr(renderer).normal_scale(1.5)
`}
lang="python"
/>
...
`Material::pbr(&renderer).await?.<setters>` rather than cloning â `Material`
...
[Light](/api/scene/light) via
[`Material::add`](/api/scene/material#add) over
raw `shader().set(...)` â the typed handles propagate updates live:

```rust collapse={1-1, 10-12}
async fn run() -> Result<(), Box<dyn std::error::Error>> {
use fragmentcolor::{Camera, Light, Material, Renderer};
