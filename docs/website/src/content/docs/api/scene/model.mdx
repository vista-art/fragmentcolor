...
Material before passing it in is cheap â `Material::clone` is an Arc-clone
...
(one per unique Mesh). For *batched* instancing â one shared Mesh, many
transforms in one draw â drop down to `Mesh::add_instance(...)` directly
...
buffer â the most recent transform-mutating call wins. Give each Model its
own Mesh.
...
by the renderer in both the opaque-batched and blend-sorted draw paths â
...
// Wide zoom level â skip the detail blobs.
blob.set_visible(false);
// Zoom back in â turn them on again.
...
The renderer reads this every frame â hidden Models are skipped in both
