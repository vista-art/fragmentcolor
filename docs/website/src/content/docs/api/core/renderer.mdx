...
Create a [Texture](/api/core/texture) from raw image bytes.

- Rust: returns a [Texture](/api/core/texture) object wrapping a GPU texture
...
Create a [Texture](/api/core/texture) from an image file path.
