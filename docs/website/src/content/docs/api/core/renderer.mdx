...
Currently returns an error on every platform â the public signature is
...
Round-trip the GPU-side bytes of a [`Shader`](/api/shaders/shader)'s
...
binding. The cheap CPU-mirror accessors â
[`Shader::get`](/api/shaders/shader/get) and
[`Shader::get_bytes`](/api/shaders/shader/get_bytes)
â reflect what was last set on the CPU side; GPU writes are not visible
through them.
...
Returns
[`RendererError::StorageBindingNotFound`](/api/core/renderer/error)
...
fn main() -> Result<(), Box<dyn std::error::Error>> { pollster::block_on(run()) }
`}
lang="rust" meta="collapse={1-1, 28-31}"
/>
