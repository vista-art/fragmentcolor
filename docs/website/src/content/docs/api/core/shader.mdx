...
FragmentColor handles std140-style 16-byte alignment for you, and large uniform blobs are pooled internally â there is nothing to configure.
...
Build a fresh [Shader](/api/core/shader) that compiles
...
the cost is one uniform buffer's worth of allocation per duplicate â not a
full pipeline recompile.

This is what [Model::new](/api/scene/model) calls
internally so each model gets its own `mesh.model` uniform slot when many
models share a single [Material](/api/scene/material).
...
// Independent copy â sets on `red` do not bleed into `template` or `blue`.
