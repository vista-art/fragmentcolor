...
Add a [Model](/api/scene/model) to this render pass.
...
fn main() -> Result<(), Box<dyn std::error::Error>> { pollster::block_on(run()) }
`}
lang="rust" meta="collapse={1-1, 33-35}"
/>
...
Absorb a scene-level [Camera](/api/scene/camera),
[Light](/api/scene/light), or any custom
[`Component`](/api/scene) into the pass. The
component is applied immediately to every shader already on the pass
(from prior [`add_model`](/api/core/pass/add_model)
calls), and re-applied to every shader added afterwards via `add_model` â
