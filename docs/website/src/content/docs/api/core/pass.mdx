...
node that implements the trait â into the pass. Each kind brings its
...
A Pass has at most one color target â call `set_target` again to swap it. Use this to render intermediate results (e.g., a shadow map) that later passes can sample.
...
A Pass has at most one depth attachment â call `set_depth_target` again to swap it.
...
To **opt out** of depth testing, simply don't call `set_depth_target` â the pass renders without depth-state, painter's-algorithm style (later draws win).
