...
// Encoded path Ã¢ÂÂ single tuple, no extra method.
const chain = TextureMipChain.prepare((encoded_png_bytes, TextureFormat.Rgba8UnormSrgb));

// Raw pixel path Ã¢ÂÂ same method, just include the size in the tuple.
...
// Hand the chain to the unified create_texture entry Ã¢ÂÂ same vocabulary.
const renderer = new Renderer();
const texture = await renderer.createTexture(chain);
`}
lang="js"
