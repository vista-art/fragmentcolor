---
title: Mipmap
description: "A pre-computed CPU mipmap chain ready to upload as a [Texture](/api/texture/texture). Build one off the renderer thread (worker, thread pool, async task, Web Worker) so the GPU thread only has to do `queue.write_texture` calls."
category: texture
...
A pre-computed CPU mipmap chain ready to upload as a [Texture](/api/texture/texture). Build one off the renderer thread (worker, thread pool, async task, Web Worker) so the GPU thread only has to do `queue.write_texture` calls.
...
- `build((bytes, format))` â encoded image bytes (PNG, JPEG, etc.); size is inferred from the decoded image.
- `build((bytes, format, size))` â raw pixel bytes already laid out for `format` at `size`.
