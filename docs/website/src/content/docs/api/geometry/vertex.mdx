---
title: Vertex
description: "A single vertex: a required 2D or 3D position plus any per-vertex attributes your shader declares. Commonly that's `uv` and `color`, but any key the shader's vertex stage reads is fair game. Build vertices and feed them into a [Mesh](/api/geometry/mesh) to draw."
category: geometry
...
A single vertex: a required 2D or 3D position plus any per-vertex attributes your shader declares. Commonly that's `uv` and `color`, but any key the shader's vertex stage reads is fair game. Build vertices and feed them into a [Mesh](/api/geometry/mesh) to draw.
