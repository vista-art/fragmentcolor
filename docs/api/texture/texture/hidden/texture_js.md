...
JavaScript-specific example for the [Texture](https://fragmentcolor.org/api/texture/texture) constructor flow. Mirrors the Rust example but writes the call directly in the JS shape (`createTexture(input, options?)`) instead of relying on the Rust→JS tuple translation.
