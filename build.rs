...
mod website {
    ...
    // Keep the original update as a convenience wrapper
    #[allow(dead_code)]
    pub fn update(api_map: &ApiMap) {
        update_version_badge();
        let out = export_examples_and_pages(api_map);
        cleanup_site(&out.expected);
        write_healthcheck_aggregators(&out.ex_js, &out.ex_py);
    }
    ...
    #[allow(dead_code)]
    pub fn update_legacy(api_map: &ApiMap) {
        use std::collections::{BTreeMap, HashSet};
        let root = meta::workspace_root();
        let docs_root = root.join("docs/api");
        let site_root = root.join("docs/website/src/content/docs/api");
        ...
        fn build_tabs_for_example(
            ...
            if !swift_abs.exists() {
                let _ = super::meta::write_if_changed(
                    &swift_abs,
