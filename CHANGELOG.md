...
- [x] **Per-Model transform rides the per-instance vertex-attribute stream.** Locations 3..6 carry the four `vec4<f32>` columns of `mat4x4<f32>`; `Model::sync_transform` writes them into the Mesh's instance buffer (clear + add a single instance). The default PBR shader reconstructs the matrix in `vs_main`. This means many Models sharing one `Material` share the **same Shader Arc** — the renderer cache-hits the pipeline by `shader_hash`, one bind-group setup covers the whole group, and each Model contributes its own row to the instance stream. No per-Model shader duplication, no per-Model uniform-buffer allocation, no `mesh.model` collisions.
- [x] **`Pass::add_model(&model)`** dedupes both the shader-attach and the mesh-attach: many Models with the same Material → one shader entry on the pass; the same Mesh attached twice → one entry on the shader. Without this the renderer would draw each mesh once per duplicate.
...
- [x] **Tests:** unit tests covering Material defaults, builder setters, shallow-clone share semantics, custom-shader silent no-op, Model transform composition (identity, pre-mult translate, post-mult rotate/scale, zero-axis rejection, set_transform sync), per-Model instance-buffer correctness, and Pass::add_model dedupe of shared shaders. 213 lib tests, 116 doctests, all passing.
- [ ] **RemixBrush migration commit (separate repo)** — adopt `Model::new(blob.mesh, Material::pbr()?.base_color(blob_color))` for the impasto-blob pipeline; replace the hand-rolled per-blob uniform plumbing. No FC-side change needed.
...
- [ ] **glTF loader.** Coming in a separate commit this cycle. `Material` field set matches glTF 2.0 PBR-MR exactly so no migration cost.
- [ ] **Batched instancing across Models that share a Mesh+Material.** Today the Mesh's instance buffer is overwritten by each Model's `sync_transform` — last writer wins for the shared-Mesh case. For true batched instancing, callers drop down to `Mesh::add_instance(...)` directly with a `Material::custom(shader)` that reads the instance attributes. A future enhancement could let `Pass::add_model` accumulate instances on a per-(Mesh, Material) bucket inside the Pass instead of mutating the Mesh.
