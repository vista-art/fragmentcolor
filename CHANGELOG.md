...
Closes the v0.11.2 follow-up "Camera object is planned" from the `Material::pbr` doc. Two new top-level types under `src/scene/`, paired with a small `Component` trait that lets Materials absorb them generically. Together with the existing `Material` they cover the three uniforms every 3D render needs (`camera.view_proj` + `camera.position`, `light.direction` + `light.color`, plus the material factors) without callers having to know the uniform paths.

- [x] **`Camera`** at `src/scene/camera.rs`. Constructors `Camera::perspective(fovy_radians, aspect, near, far)` and `Camera::orthographic(left, right, bottom, top, near, far)` (both built on glam's `*_rh` builders, matching wgpu's `[0, 1]` NDC depth range). Chainable `look_at(eye, target, up)` sets the view matrix and caches the world-space eye position. Accessors: `view_proj() -> [[f32; 4]; 4]` (column-major `proj * view`), `position() -> [f32; 3]`. State lives behind an `Arc<CameraObject>`, so a single Camera handed to many Materials propagates its `look_at` updates to all of them — no second `add` call.
- [x] **`Light`** at `src/scene/light.rs`. `Light::directional(direction, color)` is the directional constructor; accessors `direction()` and `color()`; setters `set_direction(...)` and `set_color(...)` mutate the Arc-shared state and propagate to every Material that absorbed the Light. Directional only for MVP — the type name reserves the abstraction for point / spot follow-ups.
- [x] **`Component` trait** at `src/scene/component.rs`. Camera and Light implement it; `apply(&self, shader: &Shader)` seeds the shader's uniform surface with the component's current state and registers the shader (as a `Weak<ShaderObject>`) for live updates. Custom components — any type that produces a fixed set of uniforms — can implement it too. The propagation list compacts on every update by dropping `Weak` entries that no longer upgrade, so Materials that go out of scope don't leak through their attached components.
- [x] **`Material::add<C: Component>(&self, c: &C) -> &Self`** is the public injection point. Chainable: `material.add(&camera).add(&sun)`. Internally calls `Component::apply`; the older standalone `Camera::bind` / `Light::bind` are gone.
- [x] **Cross-language bindings** for both types on Python / wasm-bindgen / uniffi (Swift, Kotlin). `Material` exposes typed `add_camera` / `add_light` shims (the trait can't cross the FFI boundary), and `Light` exposes `set_direction` / `set_color` for live mutation. Camera's `look_at` is callable repeatedly across all four languages — the Arc-shared backing means the second call propagates to every Material that absorbed it.
- [x] **Docs:** new `docs/api/scene/{camera,light}/` group + `docs/api/scene/material/add.md`. `docs/api/scene/_index.md` extended with Camera and Light after Model and Material. `docs/api/scene/material/pbr.md` and `docs/api/scene/material/shader.md` lead with `material.add(&camera).add(&sun)` instead of raw `material.shader().set("camera.view_proj", ...)` setup.
- [x] **Tests:** Camera covers projection sanity (perspective produces a non-trivial mat4, orthographic preserves `[3][3] = 1`), `look_at` view change + cached position, `material.add(&camera)` round-trip through the PBR uniform surface, **and a live-propagation test** — mutating the Camera after `add` updates the Material's shader without a second `add`. Light mirrors the same shape with `set_direction` / `set_color` propagation. 231 lib tests + 134 doctests, all passing.
