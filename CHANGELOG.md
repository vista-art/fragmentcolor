...
- [x] **`Material::clone` is deep.** Routes through `Shader::duplicate` (new) so a template Material can spawn N independent Models without per-Model `mesh.model` uniforms colliding. The underlying GPU pipeline is shared by hash; only the uniform buffer reparses.
- [x] **`Shader::duplicate` + `ShaderObject::source: Arc<str>`.** New public method spawns an independent uniform-storage copy from the same WGSL source; the source is preserved on the ShaderObject so duplication can't fail. Bound on every platform (Python / wasm / uniffi).
- [x] **`Pass::add_model(&model)`** mediates the three-step attach (push the Model's shader, add the Mesh to that shader, transform already synced) in one call. Falls through to `add_mesh`'s "last compatible shader" path so per-Model shaders stay isolated.
- [x] **Cross-language bindings** for `Material`, `Model`, `Pass::add_model`, and `Shader::duplicate` on Python / wasm-bindgen / uniffi (Swift, Kotlin). Builder-style chaining is Rust-only; in other languages setters mutate in place (Material is Arc-shared internally, so multi-statement setup gives the same shape without forcing a deep clone on every call).
- [x] **Docs:** new `docs/api/scene/{material,model}/` group (16 + 9 method pages, both `_index.md` group orderings), `docs/api/core/pass/add_model.md`, `docs/api/core/shader/duplicate.md`.
...
- [x] **Tests:** unit tests covering Material defaults, builder setters, deep-clone independence, custom-shader silent no-op, and Model transform composition (identity, pre-mult translate, post-mult rotate/scale, zero-axis rejection, set_transform sync, template sharing across two Models, Pass::add_model integration). 212 lib tests, 117 doctests, all passing.
- [ ] **RemixBrush migration commit (separate repo)** — adopt `Model::new(mesh, Material::pbr().base_color(blob_color))` for the impasto-blob pipeline; replace the hand-rolled per-blob uniform plumbing. No FC-side change needed.
- [ ] **Texture sampling in the default PBR shader (next).** MVP ships factors-only — `material.base_color`, `material.metallic`, `material.roughness`, `material.emissive`. Texture setters store the texture under the canonical glTF binding name (`base_color_map`, `metallic_roughness_map`, `normal_map`, `occlusion_map`, `emissive_map`); the default shader does not yet sample them. Binding names are stable so this is forward-compatible — current Material::custom shaders that sample them already work.
- [ ] **`AlphaMode` + `double_sided` pipeline state.** Not yet wired through the renderer; need pipeline-builder plumbing. API placeholders deferred to keep this surface lean.
- [ ] **`Scene` object.** Collection of Models with traversal / sort / light management. Currently `src/scene/` only houses `Model`; the module name reserves the spot.
- [ ] **glTF loader.** Coming in a separate commit this cycle (same chat session structure as this one). `Material` field set matches glTF 2.0 PBR-MR exactly so no migration cost.
- [ ] **Batched instancing for Models sharing Material+Mesh.** Today's path is N draw calls for N Models (RemixBrush's 279 blobs = 279 draws). The API doesn't change when this lands — Pass::add_model gains a fast path internally.
